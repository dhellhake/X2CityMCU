...
**Draft revision 1.9 — 2026-09-16**, based on approved **DEC-001-R1.0**. Records vehicle decisions only. Earlier decisions retain their status; their reviewed vehicle requirement elaborations remain approved through REQ-001-R2.0. This working decision document is not released, and owner confirmation does not establish physical verification.

[PD-001 Draft1.5](../../README.md) owns project scope, missions, performance and constraints; [ID-001](ID-001_Item_Definition.md) owns item context; [REQ-001](REQ-001_Requirements.md) owns approved obligations and planned verification. [Index](README.md) defines source/status conventions. PD-001-R1.1 and REQ-001-R2.0 are the historical releases; current PD/ID/DEC changes are Draft and do not revise those snapshots. [Git history](README.md#release-and-history) retains their records.
...
Recovered energy above 80% up to 90% is available for normal propulsion, not reserved. PD RQ-001 still requires at least 70 km for a new battery on level ground at +20 °C and 130 kg within **20–80% actual SOC**; energy outside this window is excluded and preliminary sizing takes no regeneration credit. The 90% headroom does not enlarge qualification.
...
The 10% cutoff is positive-propulsion protection, not vehicle shutdown or an all-load battery floor. HMI/lighting continue while battery protection permits (DEC-FLT-009). **Derived continuity:** restart alone is not actual-SOC recovery and cannot bypass the reached cutoff; restart/battery-handling realization remains open. The no-persistent-failure-history rule concerns faults, not this low-SOC condition. RQ-001 and regenerative windows remain DEC-REG-001; auxiliary energy, storage margins and all-load protection remain downstream.
...
While powered and communicating, report battery-current magnitude in every VD18MT controller frame: byte 4 is unsigned 0–51 A in 0.2 A increments, so transmit `min(abs(current), 51 A)` for both directions and raw zero when current is unavailable/invalid. The display value has no direction and is not control/protection evidence. Validity, accuracy, rounding and update bounds remain `WS-OI-009`; no sensing realization or reporting period is selected.
...
Retain 0x06 for both hot/cold faults, including battery temperature; DEC-FLT-003 defines its first-detected group membership above 0x01. The within-limit cold/weather allowance in PD does not authorize operation after a recognized limit violation or introduce a pre-limit derating curve. Applicable component limits/margins, recognition and finite response remain WS-OI-019. Source: owner's final thermal-reaction clarification, 2026-09-09.
...
Allow propulsion but restrict regeneration when valid cell temperature is in the qualified discharge envelope but outside the charging envelope: for the 35E source limits, −10°C ≤ T < 0°C or 45°C < T ≤ 60°C. This is an ordinary restriction, with no new indication and recovery under DEC-REG-003; it neither relaxes charging limits nor replaces faults for invalid information, operating-limit violations or actual impermissible charge transfer. Conservative integrated limits and coverage remain `WS-OI-005/019`.
...
Loss or invalidity of previously qualified actual-output information requires both-sign torque inhibition until a clean restart and fresh Ready checks. Use `0x05` unless a specific code applies; restoration alone cannot release it. Initial qualification remains a startup/self-test matter. Neither unqualified output nor zero request proves absent electrical braking; DEC-LGT-002 controls the independent light response. Observation method, coverage and response bounds remain `WS-OI-001/006/007/010/016`.
...
Owner-confirmed 2026-09-10: both the **168 h fitted outdoor** and **672 h detached indoor** storage duties require entry at **normal full charge, 80% actual SOC**, subject to the qualified conservative preparation/control tolerance. Entry below that normal-full condition or above 80% is outside these duration qualifications; entry up to 90% after regeneration is not included. No automatic forced-discharge function is selected. These conditions govern the two unattended storage duties, while ordinary parking/handling retains its normal protection requirements.

Normal temperature conditioning and, if needed, energy preparation may precede post-storage riding/function qualification; establish stored condition first. No immediate Ready, fixed remaining range or restored energy without replenishment is promised. Storage limits, load/self-discharge budget, SOC uncertainty and environmental exposure still require engineering qualification. This closes the entry/preparation choice, not physical storage acceptance; trace STO-001/002, ENV-003 and OI-056/061/063.
...
Select Hall-sensored sinusoidal FOC: three low-side phase-current shunts, centre-aligned complementary SVPWM, `Id*=0`, calibrated signed `Iq*`, d/q PI with vector saturation/anti-windup, and bounded Hall-sector angle estimation. It consumes the signed wheel-torque request and selects no speed servo, holding torque, forced startup alignment/rotation, sensorless fallback, field weakening or MTPA. [MCD-001](../Motor/MCD-001_Hall_Sensored_FOC_Technical_Design.md) is canonical.

The owner selected the unmodified **DRV8300DRGE-EVM** as `HC-TRACTION-POWER` and WeAct STM32H723VGT6 V1.2 as the sole `HC-CONTROLLER`; their [EVM integration record](../Architecture/DRV8300DRGE-EVM/DRV8300DRGE-EVM_Integration_and_Requirements_Fit.md) and [allocated traction HSI](../Architecture/DRV8300DRGE-EVM/DRV8300DRGE-EVM_WeAct_STM32H723VGT6_Traction_HSI.md) are canonical. The independent external gate inhibit remains required.

Hall map/alignment, current/power direction, physical output, protection and 40-km/h headroom remain qualification gates. The reported 9.5 rpm/V implies about 55 V no-load at 40 km/h versus 50.4 V nominal pack voltage; 58.8 V is not normal 80%-SOC operation. Failure requires controlled change, not target reduction.

<a id="dec-runtime-001"></a>
### DEC-RUNTIME-001 — Vehicle runtime realization boundary

Logical `SC-*`/`U-*` ownership is separate from Rust crates, tasks and interrupt handlers. `U-PLATFORM-BINDING` owns platform/vector/DMA binding; `U-ANALOG-ACQ-REGULAR` and `U-ANALOG-ACQ-FAST` own separate raw regular and direct injected ADC records; `U-HALL-CAPTURE`/`U-HALL-POSITION` own Hall capture through calibrated/aligned electrical position; `U-MOTOR-PHASE-FAST` and `U-SUPPLY-FAST` own their respective qualified feedback conversions; `U-TRACTION-CONTROL` forms next data; and `U-TRACTION-OUTPUT` is the only CCR/JSQR writer. The direct traction path is outside cyclic scheduling and blocking transport, uses short checked access scopes only, and hardware break/inhibit remains independently effective. The [runtime integration contract](../Architecture/SoftwareArchitecture/Runtime_Integration_Contract.md) owns the detailed allocation and source limits; no implementation or physical-protection acceptance is implied here.
...
| `WS-OI-009` | HMI/settings integration; OI-032 | Qualify charge display/current encoding, setting interpretation, visible faults and DEC-FLT-010 ordering. Verify valid/special requests, current fallback/recovery and accuracy/update bounds. DEC-LVL-003/DEC-HMI-007 fix no cross-boot, Ready or pending indication. |
| `WS-OI-010` | Fault/communication/reset behavior; OI-049 | Complete detectable-fault coverage, self-tests, reset/brownout behavior, finite response/reporting bounds and protective realization. Verify startup/late qualification, persistent/new faults after restart, communication/pending settings and physical outcome; parser timing is not a vehicle deadline. |
...
| `WS-OI-013` | Performance/range qualification; PERF-001–004 / RQ-001 / OI-052–059 / OI-063 | DEC-PERF-001, DEC-RNG-001 and DEC-RNG-002 fix level and nominal trace. Qualify physical inputs, trace tolerance, launch/rollback, adverse weather, storage margins, just-below-40 acceptance, 20–80% endpoint and descent/braking control; calculations are not test evidence. |
...
| `WS-OI-017` | Low-SOC transitions and energy; OBJ-014 / CON-048–050 / OI-041 / OI-053 / OI-061 / OI-063 | Derive SOC margins, transition/ramp bounds, repeated-boundary acceptance, all-load/post-cutoff energy and RQ-001 compatibility. Qualify DEC-STO-001/002 80%-entry storage duty, BMS/poll loads, self-discharge and post-storage condition. |
| `WS-OI-018` | SOC information/battery faults; VS-003–004 / OI-032 / OI-041 / OI-049 / OI-061 | Qualify SOC trust, age, initialization, diagnosis, actual-unit event mapping and finite response, including display fallback, reset, temperature overlap and auxiliary boundaries. Assess BMS RSOC/current/group/probe/protection data; estimator and power allocation remain open. |
| `WS-OI-019` | Temperature limits/information; PERF-007 / OI-023 / OI-025 / OI-041 / OI-049 | BAT-001 cell source limits are discharge −10…60°C and charge acceptance 0…45°C. Derive integrated limits, coverage, validity, fault boundary, response and supply/isolation/regenerative-energy protection. DEC-TMP-003 covers discharge-only conditions; known violations use 0x06 and invalid data alone do not establish one. |
| `WS-OI-020` | System-requirements completion points and allocation | Prior requirements releases retain their scope in [Git history](README.md#release-and-history). **B remains OPEN:** [ARCH-001/002/003](../Architecture/SystemArchitecture/ARCH-001_System_Architecture.md) provide the initial vehicle logical/HW/SW decomposition, component interfaces and host roles. Complete parent coverage, numerical contracts, protection/measurement mechanisms and component/host acceptance; physical feasibility, diagnostic/protection and verification gates remain. |

...
| Attribute | Value |
|---|---|
| Revision / status | 1.0 Released, 2026-09-13; **FC-001-R1.0** functional-concept baseline |
...
| Scope | Complete vehicle and EPCS across installed riding, handling, removed-pack charging/storage and service; the same physical battery crosses configurations |
| Related work | [Exact requirement/function trace](FC-002_Function_Trace.md), [logical architecture](ARCH-001_System_Architecture.md), [working requirement catalogue](../Requirements/REQ-001_Requirements.md), [released goals](../Safety/SG-001_Safety_Goals.md) |

## Purpose and interpretation

This concept answers **what functions cooperate to deliver the required behavior and safety outcomes**. Functional decomposition connects requirements, behavior and interfaces before detailed realization; NASA describes this role in its [logical-decomposition guidance](https://www.nasa.gov/reference/4-3-logical-decomposition/). The safety part connects the released goals to functional responses, conditional safe states and derived requirements, within the existing ISO-inspired tailoring; ISO Part 3 identifies the [functional safety concept as a concept-phase work product](https://www.iso.org/standard/68385.html). No formal ASIL or conformity claim is introduced.

The 19 functions below are behavior/responsibility views of the existing logical architecture. A function may involve several realization components; functional decomposition is neither a software task list nor proof of independent protection. Existing approved requirement targets and software/hardware allocations are retained. The named logical owner records the proposed coordinating responsibility for each complete function; allocated policy/interpretation leaves supply their explicitly limited contributions. These ownership bindings and the wider structural/behavioural architecture remain provisional, except for the requirement bindings already approved in R1.6.

**Authority:** released requirements and safety goals remain controlling. This release approves the functional definition, contracts, cooperation, modes and response strategy with the qualifications in the [release record](#release-record). The 12 referenced FSC refinements remain explicitly Draft in REQ-001 Draft1.7; no requirement-status promotion or numeric default follows from the concept release. The trace distinguishes a function's contribution from a requirement derivation parent. Fixed cells/BMS, VD18MT, shared two-wire coded brake interface and original mechanical lock remain given. HARA's broad context assumptions remain assumptions. Hardware hosts, sensing circuits, protection topology and software units remain downstream choices.
...
```mermaid
flowchart TB
    P["Rider-controlled transport and lifecycle use"]
    P --> R["Riding: F-001 to F-005"]
    P --> E["Energy: F-006 to F-009"]
    P --> C["Detached charge: F-010 and F-011"]
    P --> M["Mechanical and interfaces: F-012 to F-014"]
    P --> H["Information and lights: F-015 to F-017"]
    P --> S["Service: F-018 and F-019"]
```
...
Required self-tests and detection are distributed across the functions providing observations or physical effects. F-003 owns riding-session inhibition, F-010 owns charging-session inhibition, and F-009 coordinates energy protection; F-019 reports their state. No single diagnostic display or policy function supplies all fault coverage. Known valid-code aliases, shared references, BMS group/probe limitations and physical output faults remain in the fault envelope even when observation cannot distinguish them.

### Function contracts

Each row states one coordinating logical owner, principal inputs/outputs, applicable modes and unavailable/fault behavior. Numeric ranges, timing and verification criteria follow the linked requirements and the budgets below. The [record-level trace](FC-002_Function_Trace.md) identifies every supporting requirement and safety goal.
...
| <a id="f-006"></a>F-006 — Qualify battery observations and identity · LE-INPUT | Selected BMS/source observations, effective configuration and operation/reset context → qualified pack voltage, signed current, actual SOC, 14 group voltages, available/required temperature/path status and source uncertainty. | Fitted, removed charge/storage where available, startup and service. LE-BMS-LINK performs the allocated UART interpretation; the integrating function also needs truthful physical sources and configuration correspondence. Invalid/missing required data cannot authorize dependent transfer. A vehicle-side host is not required for detached operation; required local data/protection must still be provided or charging inhibited. |
| <a id="f-007"></a>F-007 — Determine battery capability · LE-ENERGY | F-006 observations, qualified component/configuration limits and operation context → separate charge/discharge limits, restrictions, readiness and recognized battery-fault information. | LE-BAT-POLICY supplies allocated policy. Apply cell/group/current/temperature constraints, not only pack SOC. Discharge-valid/charge-invalid temperatures preserve otherwise permitted propulsion and restrict regen. Consumer session latches and regen re-entry are owned by F-003/F-004/F-010; restoring a capability does not clear them. |
...
| <a id="f-009"></a>F-009 — Bound hazardous energy and interfaces · LE-ENERGY | Physical pack, traction, charger, auxiliary and accessible-path conditions; qualified protection status, configuration and fault envelope → assigned physical protective action, bounded remaining energy and truthful protection status. | All configurations, including controller/UART/supply absence, generated wheel energy, unknown path state and latent pack faults. Required protection remains effective for energized paths within the declared envelope. Detection, prevention and physical exposure limitation cover their assigned cases; do not assume all faults are observable or that a path report proves isolation. F-005/F-011/F-014/F-018 supply domain effects. |
| <a id="f-010"></a>F-010 — Govern charging sessions · LE-CHARGE-POLICY | Qualified battery/source/current activity, charge-connection/USB events, initial charge need and controller/reset/fault context → charge-enable intent, retained eligibility/completion and Waiting/Charging/Completed/Fault modes. | Removed-pack operation. New session below 80% may start when conditions qualify; initially full stays complete despite later SOC decline. Ordinary cold/warm/source waiting resumes automatically. Actual fault needs the released recovery event and fresh checks. Controller restart clears past fault history, shows Waiting during checks and preserves completion/initial eligibility; BMS reset is not a new session. Enable derives from qualified session/conditions without requiring prior charging activity. Activity qualifies the Charging indication and actual-transfer diagnosis separately from intent. |
| <a id="f-011"></a>F-011 — Convert and regulate external charging energy · LE-CHARGE | Qualified USB source/cable capability, F-010 intent and F-007 charge envelope → bounded actual battery charging, physical source/activity evidence and visible four-state indication. | Dry detached use without powered vehicle/HMI. Support net charging from 9 V / 2 A and compatible higher profiles within the 140 W input/negotiated limits. Withdraw forbidden transfer within qualified bounds, report actual faults/activity and cooperate with F-009. Status must match qualified activity and F-010 state, not merely a nonzero request. |
...
| <a id="f-016"></a>F-016 — Determine lighting modes · LE-LIGHT-POLICY | Current-session normal-light request and qualified lever/actual electrical-braking state → front On/Off and rear Off/Dim/Full intent. | Initial normal request Off; preserve last valid request during VD link loss. Rear Full for either lever, active electrical braking or either unqualified state. Return requires qualified both-released/no-electrical-braking. Passive slowing alone is excluded. Policy output does not prove physical brightness. |
| <a id="f-017"></a>F-017 — Produce vehicle lighting · LE-AUX | Protected supply and F-016 intent → physical front illumination/rear dim/full signaling and qualified integration/diagnostic evidence where observable. | Powered startup, riding, torque-inhibited and shutdown transitions wherever protection permits. Provide the required unknown-state startup output before qualified braking information exists. Actual visibility/dim-full distinction, response and supply-loss behavior require acceptance; no indication is promised without usable power. |
| <a id="f-018"></a>F-018 — Enable controlled service and safe return · LE-SERVICE | Task, physical configuration/energy state, component condition and effective configuration plus diagnostics → controlled access/isolation or explicitly energized task conditions, inspection/configuration acceptance and return-to-operation evidence. | Inspection, replacement, isolated and energized diagnostics, post-storage/crash and reassembly. Account for wheel generation and residual heat/energy. Service completion grants no torque/charge permission; normal riding or detached charging guards apply. Replacement does not reset the vehicle-life clock. |
| <a id="f-019"></a>F-019 — Present current service information · LE-SERVICE-INFO | Current producer-tagged observations, self-test results, effective identities and session states → accessible valid/unknown and incomplete/pass/fail diagnostics. | Applicable powered/service configurations. Distinguish current fault, retained inhibition, selected report, requested output and actual output. Producer reset invalidates affected old-context observations. Reporting or telemetry return cannot clear latches, prove isolation or create a charging session; no persistent onboard fault logger is introduced. |
...
The diagrams show information and physical interactions in their applicable configuration. They do not imply that riding and removed charging occur simultaneously, that arrows are separate wires, or that protection is necessarily implemented by software.
...
```mermaid
flowchart LR
    USB["External USB-C source / cable"] --> Convert["F-011 qualified source, conversion and visible status"]
    Convert <-->|physical charge energy| Pack["F-008 same removed pack"]
    Pack --> Obs["F-006 qualified battery data"]
    Obs --> Cap["F-007 charge capability"]
    Cap --> Policy["F-010 session / completion / fault policy"]
    Convert -->|events and actual activity| Policy
    Policy -->|enable and status intent| Convert
    Protect["F-009 protection"] --- Pack
    Protect --- Convert
    Access["F-014 accessible interfaces"] --- Pack
    Access --- Convert
    Service["F-018 controlled service"] <--> Info["F-019 current service information"]
    Obs --> Info
    Policy --> Info
    Protect --> Info
```
...
| IF-A-007 | F-011/006/007 ↔ F-010: qualified charging source/activity/events and enable/four-mode intent |
| IF-A-008 | All diagnostic/configuration producers → F-019 → F-018/maintainer: current observations, identity, context and unavailable states |
...
| IF-A-010 | F-008 ↔ F-005 or F-011; F-008 → F-017/other loads: physical energy, current/voltage/thermal bounds and available path evidence |
...
| IF-A-013 | F-014 ↔ pack/vehicle/charger/handler and F-018: mechanical retention/access, electrical/thermal exposure and configuration-dependent service boundary |

F-001/F-006 qualification must be possible without granting torque or unsafe charge. Initial observations are obtained under safe acquisition/test conditions; F-005/F-011 do not require a hazardous actuation merely to report ineligibility or qualified inactivity. If a required startup check needs energized motion/transfer, F-018 and the physical safety functions must establish its separately controlled conditions; the normal Ready guard cannot be bypassed. Positive propulsion and an eligible charging transfer can begin from qualified inactivity. Applied-positive-output evidence gates post-stop regeneration re-entry, while charging activity qualifies the Charging indication; neither is a prerequisite for its own first permitted transfer. Unauthorized-output monitoring/protection continues without command acceptance; implementation dependencies and coverage still require qualification.

## Modes and transition rules

These are externally relevant behavioral conditions, not a prescribed software state machine. Vehicle-off may coexist with removed-pack charging. F-008/009/014 and mechanical controls remain relevant when normal software is unpowered. The item definition retains the canonical VS-001–012 names.
...
|---|---|
| VS-001 / VS-010 — isolated or controlled energized service | F-018 establishes the task's actual power/access conditions with F-009/014, including stored/generated energy. F-019 distinguishes observations from permissions. Energized diagnostics are not normal riding or a new charging session. |
| VS-002 / VS-012 — fitted/off or detached storage | F-008 budgets all losses; F-009/014 retain assigned protection and environmental/access functions. A detached charger may have no power/indicator. Establish stored condition before conditioning or recharge can mask degradation. |
...
| VS-007 — new charging session | F-011 qualifies source and a connection/USB power event; F-006 establishes initial charge need for that event. F-010 treats initially ≥80% as complete; below 80% is eligible incomplete, waiting until all required conditions qualify. F-011 permits physical charge only within F-007/009 bounds. |
| Ordinary charging cold/warm/source wait | F-010 retains incomplete eligibility and resumes after all required source/battery conditions qualify; F-011 and F-009 enforce safe actual transfer. Completion/initial-full hold and actual-fault inhibition are not cleared by temperature or telemetry recovery. |
| Charging-controller restart / BMS-only reset | Charging-controller restart discards past fault history and shows Waiting through fresh checks while retaining completion/initial eligibility. BMS-only reset requalifies affected data and clears no session latch. F-009 bounds energy during reset and repeated retries. Shared reset affects each domain according to its own rule. |
...
| SG-007 — visibility/information | F-001/003/005/015/016/017/019 | FSC-012 adds actual signal/visibility acceptance. Existing HMI limits and truthful service information remain supporting constraints; no new indication is implied. |
...
| Detached charging or service | F-010/011 bound physical transfer and qualify restart; F-018/009/014 establish permissible access. Service access or a producer/BMS reset cannot open a session or clear another domain's latch. Remaining energy and protective function must be assessed before removing supply. |
...
| <a id="fc-s-008"></a>FC-S-008 — automatic charge and completion | F-011 qualifies 9 V / 2 A or higher permitted source; F-010 retains charge need at the connection event. Initially/full-completed sessions remain complete through SOC drop; reconnection/qualified USB cycle opens a fresh assessment. Observe actual transfer and four-state output separately. | HE-022, HE-023, HE-024 |
| <a id="fc-s-009"></a>FC-S-009 — temperature and source waiting | F-006/007 distinguish discharge-valid/charge-invalid temperature from actual thermal fault. F-010 resumes eligible incomplete charging after cold/warm/source conditions clear; F-009/011 enforce limits through cold-to-warm condensation and data uncertainty. | HE-021, HE-022, HE-024, HE-028 |
| <a id="fc-s-010"></a>FC-S-010 — reset combinations | Reset riding controller, charging controller and BMS separately and together; F-003/010 preserve their distinct lifetimes, F-006 requalifies source context, F-009 bounds repeated-pulse energy and F-019 reports current states without a fault-history log. | HE-001, HE-024, HE-025, HE-033, HE-035, HE-036 |
| <a id="fc-s-011"></a>FC-S-011 — storage and latent damage | F-008/009/014 support fitted 168 h and detached 672 h storage at qualified normal-full entry; include residual loads, weather, internal fault and unsafe depleted-entry misuse. F-018 establishes stored condition before recharging/conditioning. | HE-026, HE-027, HE-028, HE-030, HE-038 |
...
| <a id="fc-s-013"></a>FC-S-013 — maintenance and life | F-018 controls isolated/energized tasks and configuration/replacement acceptance; F-019 keeps unknown/current/latched/actual states distinct. Inspect steering/brakes/retention after wear or crash; return uses F-003 or F-010 guards without resetting the vehicle-life origin. | HE-002, HE-015, HE-016, HE-019, HE-025, HE-029, HE-031, HE-032, HE-033, HE-034 |
...
**Document review — 2026-09-13:** bounded riding, energy and lifecycle reviews plus integration walkthroughs found no remaining conflict with released behavior. Checks confirmed exact coverage of 188 catalogue records, 11 goals and 39 hazardous events, 19 function owners, 13 interface identities, valid local links/anchors and acyclic explicit requirement derivation (76 retained and 23 new parent edges). The 176 released catalogue records and both released safety documents remain unchanged. Diagram/table structure was inspected; no physical tests were executed.

No owner behavior choice remains open for this released functional-concept package. The existing conservative HARA context remains effective. The following evidence and approval gates remain explicit; they are not silently moved to a later date by producing this document.
...
| Architecture and realization · engineering | Refine project-developed composites, allocate remaining leaves, software hosts and physical endpoints, demonstrate coverage/independence where credited, and complete resource/timing contracts under WS-OI-020(B). F-009 is a distributed responsibility; no universal protection controller is selected. |
...
**FC-001-R1.0 / FC-002-R1.0 — released 2026-09-13 by Dominik, project owner.** The owner confirmed: “I have reviewed the concept. Release it.” This approves the functional definition and its trace for downstream architecture refinement, safety-requirement derivation and verification planning.

**Scope:** FC-001's 19 functions, contracts, cooperation/interfaces, modes, functional response strategy, conditional safe conditions, symbolic budgeting approach and 16 document walkthroughs; FC-002's contribution trace accounts for all 188 catalogue records and 11 released safety goals. The walkthroughs cover all 39 released hazardous events. Functional ownership records the proposed responsibility mapping; the final logical structure and its behavioural models remain open.

**Supporting snapshots:** ARCH-001 Draft0.3 and REQ-001 Draft1.7, including the three FSC clusters, are captured in this commit with their existing Draft statuses. The 12 FSC records remain proposed requirement refinements; this is not a REQ-001-R1.7 release. The 176 R1.6 catalogue records, HARA-001-R1.0 and SG-001-R1.0 retain their prior status and technical content. Other source facts and assumptions retain their original qualifications.

**Remaining gates:** architecture completion B, numerical safety/response envelopes, diagnostic/protection coverage, realization mechanisms/hosts, physical verification and residual-risk/vehicle acceptance remain open. This release establishes no formal ASIL or ISO 26262 conformity, fully qualified functional safety concept, or vehicle-use authorization. Existing evidence gates are unchanged.

**Release checks — passed, 2026-09-13:** reviewed function/requirement/scenario rows and diagram bodies are unchanged; all 188 catalogue records, 19 functions, 11 goals, 39 hazardous events and 13 interface identities reconcile. The explicit derivation graph is acyclic with 37 child records and 99 parent edges (76 retained, 23 proposed FSC edges). Local links/anchors, table structure and whitespace checks passed. Bounded release-scope and inventory reviews confirmed the status boundary above. No runtime or physical test was executed for this documentation release.

**Git snapshot:** the commit titled `Release functional concept baseline FC-001-R1.0 and FC-002-R1.0` is the controlled package. Resolve it with `git log --all --format=%H --fixed-strings --grep="Release functional concept baseline FC-001-R1.0 and FC-002-R1.0"`. Use that commit to retrieve released content; later working revisions retain their own status. Git holds the history without duplicate release copies.

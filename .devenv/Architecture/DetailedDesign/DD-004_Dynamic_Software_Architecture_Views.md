...
sequenceDiagram
    ...
    participant Q as U-TRACTION-ACQUISITION
    participant C as U-TRACTION-CONTROL
    participant O as U-TRACTION-OUTPUT
    T8->>A: one configured half-carrier trigger
    A-->>Q: direct JEOS IRQ with both JEOS, four JDR values and identified ADC epoch
    Q->>Q: snapshot/publish immutable epoch and reject partial, stale, first, wrong-context or invalid-window record
    alt epoch and authority/Hall/configuration are current
        Q->>C: direct same-IRQ immutable valid epoch
...
The exact pins, commanded-vector-selected ADC contexts, rearm order, cache policy, PWM timing and external gate path are in the [allocated traction HSI].../DRV8300DRGE-EVM/DRV8300DRGE-EVM_WeAct_STM32H723VGT6_Traction_HSI.md). This direct path is outside the 1-ms scheduler and current intercom; [the runtime contract].../SoftwareArchitecture/Runtime_Integration_Contract.md) assigns its vector/binding, data ownership and no-blocking rule. Timer break and permit observations are not physical-output proof.

## Scheduled qualification and deferred transport

The selected scheduler can run cyclic slow work; it does not make a software component a task or supply an intercom endpoint. `U-INPUT-CONTEXT` owns DMA completed-record stability. Its direct Hall accessor is used by `U-TRACTION-POSITION` in the fast chain; scheduled qualification processes regular scans and other policy inputs. A later static intercom transport may carry a project-declared route only after its route-specific context, age, capacity and loss policy are defined.

```mermaid
sequenceDiagram
    participant PH as U-PLATFORM-HEALTH
    participant IC as U-INPUT-CONTEXT
    participant IQ as U-INPUT-QUALIFIER
    participant POS as U-TRACTION-POSITION
    participant OS as CortexOs scheduler
    participant R as Future static intercom
    participant C as Project consumer
    PH->>IC: DMA completion/error dispatch, index/status only
    IC->>IC: retain stable record or mark unavailable
    OS->>IQ: selected cyclic qualification .planned release timestamp)
    IC-->>IQ: completed regular scan with context/timestamp
    Note over OS,IQ: A delayed release is not observation/current time
    PH->>POS: fast JEOS chain needs Hall evidence
    POS->>IC: direct bounded Hall snapshot accessor
    IC-->>POS: coherent state/edge record or unavailable
...
The vehicle views elaborate `SW-I-001` through `SW-I-006` and `SW-I-008/009`. They retain the ownership and constraints of [ARCH-003].../SoftwareArchitecture/ARCH-003_Software_Architecture.md), its [retention rules].../SoftwareArchitecture/ARCH-003_Software_Architecture.md#retained-restriction-and-reset-state) and [typed interactions].../SoftwareArchitecture/ARCH-003_Software_Architecture.md#typed-software-interactions); [Vehicle startup, Ready and command acceptance].#vehicle-startup-ready-and-command-acceptance) is the canonical dynamic startup view.

The source is intentionally underdefined on event-detection mechanism, ordering across simultaneously arriving observations, timing bounds, physical activity/path evidence, self-test scope, retained-state integrity, and diagnostic coverage. These views do not resolve those remaining ordering or qualification gates.

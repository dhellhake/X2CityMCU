...
| LE-BATTERY-SUPERVISION | <a id="sc-bms-link"></a>SC-BMS-LINK | Selected BMS UART response interpretation and qualification. |
| LE-BATTERY-SUPERVISION | <a id="sc-bat-policy"></a>SC-BAT-POLICY | Battery capability, restriction and battery-fault information policy. |
...
| SC-DEMAND | Individually typed accelerator, brake, active-settings, authority, `BatteryOperatingLimits`, qualified-motion and actual-output evidence → signed requested wheel command carrying both battery current bounds | Owns demand arbitration and regeneration/restriction episodes. Pending settings never govern demand; it does not own actual torque, raw ADC, FOC, hardware or authority issuance. |
...
| SC-BMS-LINK | Read-only UART transport → field-qualified `BatteryMeasurements` | Owns protocol interpretation and host last-accepted update references only, not UART electrical safety, BMS measurement time/accuracy, vendor firmware or BMS protection. |
| SC-BAT-POLICY | `BatteryMeasurements`, compiled pack configuration and typed low-SOC retention candidate → readiness/fault, independently qualified battery current limits, actual SOC and signed pack current | `.V` owns the reached-10%-cutoff restriction state described below. It does not own hardware protection or the SC-SESSION fault latch. |
...
| <a id="sw-i-003"></a>SW-I-003 | BMS-LINK → BAT-POLICY; BAT-POLICY → SESSION and DEMAND | Data publication | IF-A-009 field-qualified `BatteryMeasurements`; IF-A-003 readiness/fault and independent discharge/regenerative-charge limits. Sensor and traction health retain their Session/Traction routes and do not fan in to BAT-POLICY. |
...
| `SC-BAT-POLICY.V` | `reached10%` restriction state, current `BatteryOperatingLimitsAssessment`, qualified actual SOC and signed pack current | typed Session, Demand and HMI routes | Only the validity-tagged restriction may be retained by the vehicle host. The assessment reference supports Demand's local freshness check and is not a BMS measurement time. |
...
| `P-DEM-BATTERY-OPERATING-LIMITS-I` | `SC-DEMAND.V` | required | `BatteryOperatingLimitsAssessment`: independent discharge/regenerative-charge current limits, SOC positive torque/speed caps and assessment reference | Demand carries both amperage bounds to traction; traction selects by actual electrical transfer, never wheel-torque sign. |
...
| `P-DEM-COMMAND-O` | `SC-DEMAND.V` | provided | Requested signed rear-wheel torque in N m, inherited discharge/regenerative-charge current bounds, validity/context, authority generation, decision sequence/timestamp and expiry timestamp | `SC-DEMAND.V` owns arbitration, not actual torque. Command age/renewal never refreshes authority and is independent of telemetry routes. |
...
| `batteryMeasurementsOut` | `BmsLink.V` | provided | `BatteryMeasurements`: qualified pack voltage/current, SOC candidate/source evidence, all 14 group voltages, available BMS temperatures and protection/path state, each fieldwise valid with host update references | Transaction/decode/publication owns protocol interpretation. Packet return or BMS wake does not restore vehicle authority or regenerative charge acceptance. |
| `batteryMeasurementsIn` / `lowSocRetentionCandidateIn` | `BatteryProtection.V` | required | Typed BatteryMeasurements and the separate low-SOC retention candidate | No temperature/supply/phase/traction observation fan-in exists. Separate discharge/regenerative candidates require their needed battery measurement evidence. |
| `batteryReadinessAndFaultOut`, `batteryOperatingLimitsOut`, `stateOfChargeOut`, `batteryCurrentOut` | `BatteryProtection.V` | provided | Readiness/fault; independent battery-side discharge and regenerative-charge amperes limits plus SOC N·m/km/h caps; qualified actual SOC; signed pack current | `reached10%` remains internal retention and BatteryProtection retains no battery-fault history. The output records do not prove physical protection. |
...
| `P-TR-COMMAND-I` | `SC-TRACTION-CTRL.V` | required | `P-DEM-COMMAND-O` signed command and both battery current bounds | A command is accepted only with valid current authority, context and configuration. Traction applies zero permitted current for an unqualified or forbidden electrical branch. |
...
| `P-TR-OBS-O` | `SC-TRACTION-CTRL.V` | provided | Aggregate output/energy/fault availability observation for Session | It reports observation, never proof that a command achieved physical torque or zero output. |
...
| `R-V06` | `BatteryProtection.batteryReadinessAndFaultOut` -> `SessionPolicy.batteryReadinessAndFaultIn` | SW-I-003 | Scheduled direct/owned snapshot; BatteryProtection owns readiness/fault | Session applies the established common fault-order rule. |
...
| `R-V12` | `BatteryProtection.batteryOperatingLimitsOut` -> `DemandPolicy.batteryOperatingLimitsIn` | SW-I-003 | Scheduled direct/owned snapshot; BatteryProtection owns operating limits | Unknown input never enlarges command permission; discharge and regenerative-charge amperes limits remain independent. |
...
| `R-V14` | `SC-DEMAND.V.P-DEM-COMMAND-O` -> `SC-TRACTION-CTRL.V.P-TR-COMMAND-I` | SW-I-004, IF-A-004 | Current command snapshot/direct acceptance; demand owns torque request and inherited current bounds | Traction rejects absent, invalid, expired, context- or configuration-mismatched command and applies the two amperage limits by actual electrical transfer. |
...
| `R-V16` | `BmsLink.batteryMeasurementsOut` -> `BatteryProtection.batteryMeasurementsIn` | SW-I-003, IF-A-003/009 | Scheduled direct/owned snapshot; BmsLink owns fieldwise measurements | Returned packet is not a restored permission or session state. |
| `R-V19` | `SC-PLATFORM.V.P-PLT-RETENTION-O` -> `SC-BAT-POLICY.V.lowSocRetentionCandidateIn` | SW-I-009, SOC-006 | Service/result; platform owns retention candidate | Only valid restored `reached10%` state is considered; no battery fault history is restored. |
...
| `R-V24` | `SC-BAT-POLICY.V.stateOfChargeOut` / `batteryCurrentOut` -> `SC-HMI.V.stateOfChargeIn` / `batteryCurrentIn` | SW-I-006, IF-A-006 | Scheduled direct/owned snapshots; BatteryProtection owns the records | Presentation fallback is not a qualified battery fact. |
...
flowchart LR
    ...
    subgraph SES[SC-SESSION.V]
        ...
        SEAO[P-SES-AUTH-O]
        SERO[P-SES-REPORT-O]
        SBR[batteryReadinessAndFaultIn]
        SSO_SOC[stateOfChargeIn]
    end
    subgraph DEM[SC-DEMAND.V]
        ...
        DBC[P-DEM-BATTERY-OPERATING-LIMITS-I]
        DMO[P-DEM-MOTION-I]
    ...
    subgraph BMS[SC-BMS-LINK.V]
        ...
        BTO[P-BATTERY-MEASUREMENTS-O]
    end
    subgraph BAT[SC-BAT-POLICY.V]
        BFI[P-BATTERY-MEASUREMENTS-I]
        BRO[batteryReadinessAndFaultOut]
        BEO[P-BATTERY-OPERATING-LIMITS-O]
        BSO[stateOfChargeOut]
        BCO[batteryCurrentOut]
    end
    ...
    BTO -->|R-V16 BatteryMeasurements| BFI
    BRO -->|R-V06 readiness/fault| SBR
    BSO -->|R-V06 actual SOC| SSO_SOC
    BEO -->|R-V12 independent current limits and SOC caps| DBC
    HLI -->|R-V21 normal request| LNR
    SERO -->|R-V23 report snapshot| HRI
    BSO -->|R-V24 actual SOC| HSI
    BCO -->|R-V24 signed pack current| HCI
...
flowchart LR
    ...
    PRO -->|R-V19 retained restriction candidate| LSRI[lowSocRetentionCandidateIn on SC-BAT-POLICY.V]
...
flowchart LR
    ...
    PVRO -->|R-V19 restore result| BP[lowSocRetentionCandidateIn on SC-BAT-POLICY.V]
```
...
**Vehicle startup.** SC-PLATFORM.V creates a new context and marks prior observations unusable. Each software component uses its owner-local immutable calibration parameters compiled into the installed firmware. A parameter change requires a software rebuild, deployment and restart; it is never a runtime message, service activation or context rollover. SC-ANALOG-ACQ.V, the named sensor interfaces, SC-BMS-LINK.V and SC-TRACTION-CTRL.V execute their allocated self-tests/qualification, including applicable parameter checks, without granting torque; SC-SET.V receives current-startup settings; SC-BAT-POLICY.V requalifies BatteryMeasurements, restores/qualifies `reached10%`, and produces readiness/fault, operating limits, actual SOC and signed current. Each producer reports pass/fail/incomplete with its context to SC-SESSION.V. SC-SESSION.V alone evaluates the simultaneous Ready guard and issues SW-I-004 authority. SC-DEMAND.V may then publish a context-matched command only with its individually usable dependencies and its compiled demand profile, which SC-TRACTION-CTRL.V must separately accept. Torque remains commanded to zero until both current authority and a valid command are accepted, and returns to zero when either expires or becomes invalid; physical zero/output protection remains the traction and energy realization responsibility.

...
| `U-TRACTION-ACQUISITION` / `U-TRACTION-CONTROL` / `U-TRACTION-OUTPUT` | Direct fast records internal to Detailed Design, including `FastEpochV1` and `FastCommandV1` | the immediately following detailed unit only | These are not SC public ports or ARCH-003 `R-V*` routes. The runtime contract and DD own the direct JEOS-to-output allocation. |
...
| `P-SET-MOTION-I` | `SC-SET.V` | required | `QualifiedMotionV1` for the standstill application guard | It is unavailable until validated criteria support physical standstill; pending state cannot replace it. |
...
| `P-SES-MOTION-I` | `SC-SESSION.V` | required | `QualifiedMotionV1` for the Ready standstill prerequisite | It is published independently of Ready and command acceptance; unknown is not standstill. |
...
| `P-DEM-ACTIVE-SETTINGS-I` | `SC-DEMAND.V` | required | `ActiveRidingSettingsV1` | Pending/requested settings cannot select a demand profile or speed cutoff. |

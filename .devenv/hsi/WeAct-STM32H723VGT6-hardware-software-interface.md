...
| --- | --- | --- | --- | --- |
| `HSI-GEN-001` | The software shall execute only on the owner-confirmed WeAct Studio STM32H7xx V1.2 board populated with an STM32H723VGT6 in the 100-pin LQFP package and fitted 0.96 inch 80x160 ST7735 LCD. | System integration | `I`, `T-HW` | Assumption; owner confirmed this configuration on 2026-09-13; pin continuity remains unmeasured. |
| `HSI-GEN-002` | The linker memory map, startup code and peripheral base addresses shall match the STM32H723VGT6 memory and peripheral map. | Linker, startup, generic drivers | `I`, `A` | Implemented in [`memory.x`](../../memory.x), [`src/drv/startup`](../../src/drv/startup) and the drivers. |
...
| `HSI-GEN-007` | A change to target part, WeAct board revision, clock, memory allocation, peripheral configuration or pin assignment shall update the owning controlled document, its parent traces and affected verification before release. | Configuration/change management | `I` | Partial; the documents are version controlled, but parent traces and release workflow are `TBD`. |
...
| ID | Requirement | Allocated element | Verification | Current status and evidence |
| --- | --- | --- | --- | --- |
| `HSI-STA-001` | Reset startup shall mask configurable interrupts and select the internal LDO supply, waiting for `PWR_CSR1.ACTVOSRDY`, before any Rust stack use, TCM state change or runtime memory write. | Reset handler | `I`, `T-HW` | Implemented by the naked `Reset` entry in [`src/drv/startup/mod.rs`](../../src/drv/startup/mod.rs), which masks interrupts, requests LDO and waits for `ACTVOSRDY` before branching to Rust. |
...
| ID | Requirement | Allocated element | Verification | Current status and evidence |
| --- | --- | --- | --- | --- |
| `HSI-CLK-001` | The naked reset entry shall select the internal LDO supply by setting `PWR_CR3.LDOEN=1`, and shall wait for `PWR_CSR1.ACTVOSRDY` before Rust stack or RAM access. | PWR | `I`, `T-HW` | Implemented by the naked `Reset` entry; the later [`ConfigureLdoSupply`](../../src/mcu/peripherals/pwr.rs) sets `BYPASS=0` and `SCUEN=0`. |
| `HSI-CLK-002` | Before selecting a 550 MHz CPU clock, startup shall select voltage scale 0 directly through PWR and wait for voltage readiness. | PWR | `I`, `T-HW` | Implemented and verified; VOS0 and readiness readback passed in the [550 MHz evidence record](evidence/2026-09-13-stm32h723-550mhz.md). |
| `HSI-CLK-003` | The system clock source shall be the WeAct STM32H723VGT6 board's 25 MHz HSE crystal on PH0/PH1 in crystal mode with HSE bypass disabled. | Board oscillator, RCC | `I`, `A`, `T-HW` | Implemented by [`src/mcu/peripherals/rcc.rs`](../../src/mcu/peripherals/rcc.rs); the physical source is specified by the [board profile](../STM32H723VGT6/WeAct-STM32H723VGT6-board.md). |
| `HSI-CLK-004` | PLL1 shall use HSE with `DIVM1=5`, `DIVN1=110`, `DIVP1=1`, wide VCO (192–836 MHz), 4-to-8-MHz input range and fractional mode disabled. PLL1P shall be enabled; PLL1Q and PLL1R shall remain disabled. | RCC PLL1 | `I`, `A`, `T-HW` | Implemented and verified by PLL1 readback in the [550 MHz evidence record](evidence/2026-09-13-stm32h723-550mhz.md). |
...
| `HSI-CLK-006` | `D1CPRE` shall divide SYSCLK by 1, `HPRE` shall divide SYSCLK by 2, and each APB prescaler shall divide HCLK by 2. | RCC bus clocks | `I`, `A`, `T-HW` | Implemented by the 550 MHz bus-clock configuration. |
| `HSI-CLK-007` | The configured clock tree shall produce a 550 MHz CPU clock, 275 MHz HCLK/AXI/AHB clock and 137.5 MHz PCLK1, PCLK2, PCLK3 and PCLK4 from a 25 MHz HSE. | RCC | `A`, `T-HW` | Register configuration and traceable clock calculation passed; independent calibrated clock measurement remains open. See the [550 MHz evidence record](evidence/2026-09-13-stm32h723-550mhz.md). |
| `HSI-CLK-008` | Before switching SYSCLK to PLL1, the Flash interface shall be configured for three wait states at the configured voltage and clock. | Flash, startup sequence | `I`, `A`, `T-HW` | Implemented and verified by `FLASH_ACR` readback in the [550 MHz evidence record](evidence/2026-09-13-stm32h723-550mhz.md). |
| `HSI-CLK-009` | Before switching SYSCLK to PLL1, `FLASH_ACR.WRHIGHFREQ` shall be set to `3` for voltage scale 0 and a 275 MHz AXI clock. | Flash | `I`, `A`, `T-HW` | Implemented and verified by `FLASH_ACR=0x33` readback in the [550 MHz evidence record](evidence/2026-09-13-stm32h723-550mhz.md). |
...
| `HSI-CLK-012` | Before switching to the 550 MHz PLL1 clock and before supervised operation, startup shall enable SYSCFG and assert `UR18 bit 0` while HSI is selected so persistent `FLASH_OPTSR2.CPUFREQ_BOOST bit 2` is set; this boost mode disables ITCM and DTCM ECC. The relevant clock, power and Flash registers shall be read back or independently checked. | MCU initialization diagnostics | `I`, `T-FI`, `T-HW` | Implemented and verified: boost provisioning, preserved option bits, `UR18`, VOS/readiness and clock/Flash readback passed in the [550 MHz evidence record](evidence/2026-09-13-stm32h723-550mhz.md). |

### Scheduler Timing, Program Flow And Watchdog

| ID | Requirement | Allocated element | Verification | Current status and evidence |
| --- | --- | --- | --- | --- |
| `HSI-TIM-001` | SysTick shall use the processor clock and shall be configured with `SYSTICK_CLOCK_HZ = 550_000_000`. | SysTick, OS | `I`, `A`, `T-HW` | Implemented and verified by register/state readback in the [550 MHz evidence record](evidence/2026-09-13-stm32h723-550mhz.md). |
...
| `HSI-TIM-005` | A scheduler deadline less than 4096 processor-clock ticks away shall not be programmed as a short SysTick interval. The driver shall install a valid maximum-length fallback and report `ImmediateRescanRequired`; the OS shall then software-pend SysTick for a fresh scheduler scan. Reload value zero shall not be used as a wakeup. | SysTick driver, SCB, OS | `I`, `T-SW`, `T-HW` | Partial: implementation and smoke behavior passed in the [550 MHz evidence record](evidence/2026-09-13-stm32h723-550mhz.md); the 4096-tick WCET bound remains unmeasured. |
| `HSI-WDG-001` | WWDG1 shall be clocked from PCLK3 at 137.5 MHz and configured with divider 32768, reload counter `0x7F`, window counter `0x61` and early-wakeup interrupt disabled. | RCC, WWDG1 | `I`, `A`, `T-HW` | Implemented and verified by register readback in the [550 MHz evidence record](evidence/2026-09-13-stm32h723-550mhz.md). |
| `HSI-WDG-002` | The WWDG1 configuration shall provide a strict earliest service boundary of 31 ticks (7.387695 ms) and reset at 64 ticks (15.252015 ms) after each reload, subject to the prescaler phase. | WWDG1 | `A`, `T-HW` | Partial: timing is calculated and the software interval is guarded; precise reset timing remains open because prescaler phase may shorten it by one tick. |
...
| `HSI-WDG-007` | The PFM shall authorize and perform WWDG1 service only between 8500 us and 14000 us relative to the current supervision-cycle start. | PFM, WWDG1 | `I`, `A`, `T-HW` | Implemented and watchdog service/reset behavior passed in the [550 MHz evidence record](evidence/2026-09-13-stm32h723-550mhz.md). |
...
| `HSI-WDG-009` | Detection of invalid flow, invalid PFM state, missing checkpoints or a missed software service interval shall latch a diagnostic fault and inhibit all subsequent watchdog service until reset. | PFM | `I`, `T-FI`, `T-HW` | Partial: missing-service watchdog reset and sustained restart passed; the complete PFM fault matrix remains open. |
| `HSI-WDG-010` | WWDG1 shall be started only after required MCU, communication, OS, component and PFM initialization has completed successfully, and before any safety-relevant cyclic application task executes. | Startup, MCU manager | `I`, `T-HW` | Partial: nominal startup and sustained scheduler operation passed in the [550 MHz evidence record](evidence/2026-09-13-stm32h723-550mhz.md); system startup completeness criteria remain `TBD`. |
...
| `HSI-WDG-014` | Before enabling WWDG1, firmware shall set and read back `RCC_GCR.WW1RSC=1` on every startup to request a complete system reset. | RCC, startup | `I`, `T-FI`, `T-HW` | Implemented and verified by `WW1RSC=1` readback and reset recovery in the [550 MHz evidence record](evidence/2026-09-13-stm32h723-550mhz.md). |
...
| `HSI-UAR-002` | USART1 shall use a 137.5 MHz PCLK2 kernel clock, USARTBRR `1194`, 115200 baud, PA9/AF7 as push-pull TX and PA10/AF7 with pull-up as RX. | RCC, GPIOA, USART1 | `I`, `A`, `T-HW` | Partial: `BRR=1194` and register configuration passed in the [550 MHz evidence record](evidence/2026-09-13-stm32h723-550mhz.md); connector-level baud/loopback remains open. |
...
| `HSI-UAR-009` | Equipment connected to GPIO through a WeAct STM32H723VGT6 board header shall satisfy STM32H723VG input/output voltage limits, share a defined ground reference and shall not back-power an unpowered participant. | WeAct board-header integration | `I`, `A`, `T-HW` | Assumption; equipment attached to the board headers is not controlled by firmware. |
| `HSI-LED-001` | PE3 shall be configured as an active-high, low-speed push-pull GPIO output without an internal pull. Its output latch shall be set low before output mode is selected. | RCC, GPIOE, heartbeat output | `I`, `T-HW` | Implemented by [`src/mcu/boardled/mod.rs`](../../src/mcu/boardled/mod.rs); the [board profile](../STM32H723VGT6/WeAct-STM32H723VGT6-board.md) maps PE3 to the active-high blue user LED. |
...
| `HSI-DBG-001` | Development debug shall use the MCU SWD interface on PA13/SWDIO and PA14/SWCLK through the board P3 4-pin SWD header, with reset requested through the debug probe or the separate board NRST button; four-wire JTAG operation shall not be assumed. | Board SWD header, MCU debug interface, debugger configuration | `I`, `T-HW` | Implemented; 3V3, ground and SWD routing are defined by the [connector reference](../STM32H723VGT6/WeAct-STM32H723VGT6-connectors.md); P3 does not expose NRST. |
| `HSI-DBG-002` | Safety-relevant operation on the WeAct STM32H723VGT6 board shall not depend on an attached debugger, and production debug access shall follow the item safety and security concept. | Board and system integration | `I`, `T-HW` | Partial; debugger independence and the production debug-access policy are `TBD`; previous-target standalone claims do not apply. |
...
[board profile](../STM32H723VGT6/WeAct-STM32H723VGT6-board.md), with physical
routes recorded in the
[connector reference](../STM32H723VGT6/WeAct-STM32H723VGT6-connectors.md).
...
| `HSI-VER-003` | Target verification shall demonstrate 550 MHz CPU timing and the derived bus clocks using an independent measurement or a traceable timing test. | Verification | `A`, `T-HW` | Partial: register readback and traceable calculation passed; independent calibrated clock measurement remains open. See the [550 MHz evidence record](evidence/2026-09-13-stm32h723-550mhz.md). |
| `HSI-VER-004` | Target verification shall demonstrate execution inside and outside ITCM, valid DTCM data/stack use, vector-table relocation and correct interrupt dispatch after relocation. | Verification | `T-HW` | Implemented behavior and all listed smoke checks passed in the [550 MHz evidence record](evidence/2026-09-13-stm32h723-550mhz.md). |
| `HSI-VER-005` | Target verification shall exercise nominal, early, late, missing, duplicate, out-of-sequence and internally corrupted PFM/watchdog cases and record the resulting diagnostic and reset timing. | Verification | `T-FI`, `T-HW` | Partial: nominal operation and missing-service reset passed; complete PFM fault matrix and precise watchdog reset timing remain open. |
| `HSI-VER-006` | Target verification shall measure baud rate and validate transmit, receive and error behavior for all configured UARTs at the connector/device boundary. | Verification | `T-HW`, `T-FI` | Open: connector-level calibrated baud/loopback and error behavior were not measured. |
...
| Internal Flash | `0x0800_0000` | 1 MiB | Vector load image, ordinary code/const data, RAM load images |
| DTCM | `0x2000_0000` | 128 KiB | 1024-byte-aligned 179-entry RAM vector table, OS/task stacks, SysTick/WWDG/PFM state, 4 KiB MSP stack |
| AXI SRAM | `0x2400_0000` | 320 KiB | Ordinary `.data` and `.bss`, including peripheral handles |
| D2 SRAM | `0x3000_0000` | 32 KiB | Exposed by linker symbols, no section allocated currently |
| D3 SRAM | `0x3800_0000` | 16 KiB | Exposed by linker symbols, no section allocated currently |
...
- Fitted ST7735 LCD SPI interface and backlight control; no display driver is implemented.
...
- [STM32H723VG product and datasheet page](https://www.st.com/en/microcontrollers-microprocessors/stm32h723vg.html)
- [STM32H723 reference manual RM0468](https://www.st.com/resource/en/reference_manual/dm00603761.pdf)

Physical-board sources are recorded with the corresponding
[board documentation](../STM32H723VGT6/README.md).

The board-level configuration was cross-checked against the [WeAct Studio MiniSTM32H723 documentation](https://github.com/WeActStudio/WeActStudio.MiniSTM32H723), including the owner-confirmed 2026-09-13 V1.2 assembly with fitted 0.96 inch 80x160 ST7735 LCD, 100-pin STM32H723VGT6 identity, 25 MHz oscillator, P3 4-pin SWD header, PA9/PA10 serial pins and PE3 active-high blue user LED. The [STM32H723VG datasheet](https://www.st.com/resource/en/datasheet/stm32h723vg.pdf) and [RM0468 reference manual](https://www.st.com/resource/en/reference_manual/dm00603761.pdf) control MCU limits and register behavior. Pin continuity remains unmeasured. Target validation is pending the [550 MHz evidence record](evidence/2026-09-13-stm32h723-550mhz.md).

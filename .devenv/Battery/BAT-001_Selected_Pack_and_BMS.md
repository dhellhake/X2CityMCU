...
These are cell limits/test references, not selected charger setpoints or proof that every cell in the built pack meets its original specification. In particular, neither 4.20 V nor 2.65 V defines a project SOC threshold.

| Derived 14S5P quantity | Calculation / meaning |
|---|---|
| Nominal pack voltage | `14 × 3.60 = 50.4 V` |
| Upper cell-test voltage reference | `14 × 4.20 = 58.8 V`; not the 80% actual-SOC charging target or a component transient rating |
...
| Ideal continuous current ceilings | Discharge `5 × 8 = 40 A`; charge `5 × 2 = 10 A`. Applies to external charge and regeneration; actual limits must account for unequal sharing, temperature, ageing, connections and protection margins |
...
No 65 A pulse capability is established. Battery DC current is not motor phase current or wheel torque. The BMS's 50 A designation cannot override the 40 A ideal continuous cell-bank ceiling, nor establish a 50 A charging allowance.
...
- **SOC and charging:** retain external 80%, regeneration 90%, low-charge 20/10% and display mapping. Voltage cannot be linearly mapped to these SOC percentages. Qualify BMS capacity configuration, RSOC calibration and uncertainty against actual charge; a vendor full-voltage calibration procedure is not authorization to exceed the external-charge ceiling. The 28 V USB input reference is below pack charging voltage; the charger must provide the necessary conversion, with topology still open.

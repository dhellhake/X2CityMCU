...
The required signal path is allocated as: physical Hall supply/conditioning → `SC-HALL-IF.V` capture integrity, map/alignment, transition/direction/timestamp and complete qualified electrical position → `SC-TRACTION-CTRL.V` traction use. It preserves source/reset context, validity and freshness; it may not authorize torque by itself. Existing Ready, torque-authority and recognized-fault rules remain controlling.

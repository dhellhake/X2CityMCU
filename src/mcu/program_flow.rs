...
use crate::{
    drv::wwdg::Wwdg,
    mcu::peripherals::wwdg::WWDG_RELOAD_COUNTER,
    os::task::Task,
};
...
impl ProgramFlowMonitor {
    ...
    pub fn ConfigureFromTasks<const TASK_COUNT: usize, const STACK_SIZE: usize>(
        &mut self,
        tasks: &[Task<STACK_SIZE>; TASK_COUNT],
        start_us: u64
    ) {
        ...
        for taskIdx in 0..TASK_COUNT {
            ...
            if !tasks[taskIdx].role.ReportsProgramFlowCheckpoints(tasks[taskIdx].cycletime) {
                continue;
            }
            ...
            let Some(period_us) = tasks[taskIdx].cycletime.period_us() else {
                self.RecordFault(
                    ...
                    ProgramFlowCheckpoint::new(tasks[taskIdx].id, ProgramFlowCheckpointKind::Unknown),
                    start_us,
                );
                return;
            };
            ...
            while release_us <= SUPERVISION_CYCLE_US {
                ...
                self.InsertTaskReleaseCheckpoints(tasks[taskIdx].id, release_us, prioritySlot, start_us);
    ...
    fn SameReleasePrioritySlot<const TASK_COUNT: usize, const STACK_SIZE: usize>(
        tasks: &[Task<STACK_SIZE>; TASK_COUNT],
        taskIdx: usize,
        release_us: u32
    ) -> u32 {
    ...
    fn StartNextCycle(&mut self) {
        self._state = ProgramFlowState::Running;
        self.SetExpectedIndex(0);
        self.SetCycleStart(self._cycleStartUs.saturating_add(SUPERVISION_CYCLE_US as u64));
    ...
    fn RelativeTimestamp(&self, now_us: u64) -> u32 {
        now_us.saturating_sub(self._cycleStartUs).min(u32::MAX as u64) as u32
    }
